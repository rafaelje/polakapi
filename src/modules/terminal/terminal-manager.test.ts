...
describe("TerminalManager CLI wiring", () => {
  ...
  it.each([
    ...
    "splits the focused pane in half toward %s without rearranging its neighbor",
    async (position, axis) => {
      const manager = makeManager();
      await manager.addPane();
      await manager.addPane();
      ...
      await splitFocusedTerminal(manager, position);
      const third = manager.focusedPaneId;
      ...
      expect(manager.ids()).toEqual([first, third, second]);
      expect(fake.attachCalls[2].opts?.cwd).toBe("/tmp/project");
    },
  );

  it("opens the first terminal when splitting an empty project", async () => {
    const manager = makeManager();
    await splitFocusedTerminal(manager, "bottom");
    expect(manager.ids()).toHaveLength(1);
    expect(manager.layoutSnapshot).toEqual({ type: "pane", paneId: manager.ids()[0] });
  });
