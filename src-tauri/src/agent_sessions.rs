...
const CODEX_TOTAL_TIMEOUT: Duration = Duration::from_secs(60);
const CLAUDE_MAX_SCANNED_LINES: usize = 200;
...
pub async fn agent_list_sessions() -> Result<AgentSessionsResult, String> {
    ...
    let (codex, claude, opencode) = tokio::join!(codex_sessions_with_deadline(), claude, opencode);
    ...
    append_provider_result(
        ...
        &mut warnings,
        "claude",
        claude.unwrap_or_else(|error| Err(format!("session discovery task failed: {error}"))),
    );
    append_provider_result(
        &mut sessions,
        &mut warnings,
        "opencode",
        opencode.unwrap_or_else(|error| Err(format!("session discovery task failed: {error}"))),
    );
...
async fn codex_sessions_with_deadline() -> Result<Vec<AgentSession>, String> {
    tokio::time::timeout(CODEX_TOTAL_TIMEOUT, codex_sessions())
        .await
        .map_err(|_| "timed out discovering Codex sessions".to_string())?
}
